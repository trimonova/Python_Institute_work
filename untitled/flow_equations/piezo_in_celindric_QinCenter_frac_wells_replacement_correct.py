...
frac_pressure = [5000000, 5000000, 5000000,5000000, 5000000, 750000, 5000000, 5000000, 5000000, 750000]
frac_angle = int(3*np.pi/4/delta_fi)
frac_angle_2 = int(np.pi/delta_fi) + frac_angle
frac_coords = [(0, frac_angle), (1, frac_angle), (2, frac_angle), (3, frac_angle), (4, frac_angle), (5, frac_angle), (0, frac_angle_2), (1, frac_angle_2), (2, frac_angle_2), (3, frac_angle_2)]
#frac_coords = []
frac_coord_1 = [0, 1, 2, 3, 4, 5]
frac_coord_2 = [0, 1, 2, 3]
frac_pressure_1 = [5000000, 5000000, 5000000,5000000, 5000000, 750000]
frac_pressure_2 =  [5000000, 5000000, 5000000, 750000]
...
print(Pres_distrib)
def PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3_water, c3_oil, c4, wells_coord, CP_dict, delta_r, frac_coord_1, frac_coord_2, frac_angle, frac_angle_2, xf, wells_frac_coords, frac_pressure_1, frac_pressure_2):
    ...
    for m in range(0, M_fi):
        ...
        for n in range(1, N_r - 1):
            ...
            if (frac_angle + xf >= m >= frac_angle - xf and n <= frac_coord_1[-1]) or \
                    (frac_angle_2 + xf >= m >= frac_angle_2 - xf and n <= frac_coord_2[-1]) or \
                    (m == frac_angle and frac_coord_1[-1] < n < frac_coord_1[-1]+xf) or \
                    (m == frac_angle_2 and frac_coord_2[-1] < n < frac_coord_1[-1]+xf):
        ...
        if m == frac_angle:
            A[frac_coord_1[-1]+xf][frac_coord_1[-1]+xf] = 1 / delta_r / mu_oil + 1 / delta_r / mu_water
            A[frac_coord_1[-1]+xf][frac_coord_1[-1]+xf+1] = -1 / delta_r / mu_water
            A[frac_coord_1[-1]+xf][frac_coord_1[-1]+xf-1] = -1 / delta_r / mu_oil

        if m == frac_angle_2:
            A[frac_coord_2[-1]+xf][frac_coord_2[-1]+xf] = 1 / delta_r / mu_oil + 1 / delta_r / mu_water
            A[frac_coord_2[-1]+xf][frac_coord_2[-1]+xf+1] = -1 / delta_r / mu_water
            A[frac_coord_2[-1]+xf][frac_coord_2[-1]+xf-1] = -1 / delta_r / mu_oil
        ...
        if m != M_fi-1:
            ...
            for n in range(0, N_r):
                if (frac_angle + xf >= m >= frac_angle - xf and n <= frac_coord_1[-1]) or \
                        (frac_angle_2 + xf >= m >= frac_angle_2 - xf and n <= frac_coord_2[-1]) or (m == frac_angle and frac_coord_1[-1] < n < frac_coord_1[-1]+xf) or \
                    (m == frac_angle_2 and frac_coord_2[-1] < n < frac_coord_1[-1]+xf):
        ...
        if m == frac_angle:
            B[frac_coord_1[-1]+xf][0] = 0
        if m == frac_angle_2:
            B[frac_coord_2[-1]+xf][0] = 0
        ...
        half_length_1 = int(len(frac_coord_1)-1)
        half_length_2 = int(len(frac_coord_2) - 1)

        if m == frac_angle:
            A[half_length_1+1][half_length_1] = 0
            B[half_length_1+1][0] = B[half_length_1+1][0] - (c1 - c2/(((half_length_1+1+1)*delta_r)))*frac_pressure[-1]
            for i in range(half_length_1, 0, -1):

                A = np.delete(A, frac_coord_1[i], axis=0)
                A = np.delete(A, frac_coord_1[i], axis=1)
                B = np.delete(B, frac_coord_1[i], axis=0)

        if m == frac_angle_2:
            A[half_length_2+1][half_length_2] = 0
            B[half_length_2+1][0] = B[half_length_2+1][0] - (c1 - c2/(((half_length_2+1+1)*delta_r)))*frac_pressure[-1]
            for i in range(half_length_2, 0, -1):

                A = np.delete(A, frac_coord_2[i], axis=0)
                A = np.delete(A, frac_coord_2[i], axis=1)
                B = np.delete(B, frac_coord_2[i], axis=0)
        ...
        if m == frac_angle:
            for i in range(half_length_1):
                P_new = np.insert(P_new, frac_coord_1[i], frac_pressure_1[i])

        if m == frac_angle_2:
            for i in range(half_length_2):
                P_new = np.insert(P_new, frac_coord_2[i], frac_pressure_2[i])
...
#-----------------------------------------------------------------------------------
    ...
    for n in range(0, N_r):
        ...
        if frac_coord_1[-1] < n < frac_coord_1[-1]+xf:
            A[frac_angle][frac_angle] = -2 * c4 / ((n + 1) * delta_r) ** 2 - c3_oil

        if frac_coord_2[-1] < n < frac_coord_2[-1]+xf:
            A[frac_angle_2][frac_angle_2] = -2 * c4 / ((n + 1) * delta_r) ** 2 - c3_oil
        ...
        if frac_coord_1[-1] < n < frac_coord_1[-1]+xf:
            m = frac_angle
            B[frac_angle][0] = -c1*(Pres_distrib[n+1][m] - 2*Pres_distrib[n][m] + Pres_distrib[n-1][m]) - \
                             c2/((n+1)*delta_r)*(Pres_distrib[n+1][m] - Pres_distrib[n-1][m]) -\
                             c3_oil*Pres_distrib[n][m]

        if frac_coord_2[-1] < n < frac_coord_2[-1]+xf:
            m = frac_angle_2
            B[frac_angle_2][0] = -c1*(Pres_distrib[n+1][m] - 2*Pres_distrib[n][m] + Pres_distrib[n-1][m]) - \
                             c2/((n+1)*delta_r)*(Pres_distrib[n+1][m] - Pres_distrib[n-1][m]) -\
                             c3_oil*Pres_distrib[n][m]
...
if __name__ == '__main__':
    for t in range(T_exp):
        print(t)
        Pres_distrib = PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3_water, c3_oil, c4, wells_coord, CP_dict, delta_r, frac_coord_1, frac_coord_2, frac_angle, frac_angle_2, xf, wells_frac_coords, frac_pressure_1, frac_pressure_2)
