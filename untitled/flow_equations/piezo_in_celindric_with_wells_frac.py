...
if __name__ == '__main__':
    ...
    frac_pressure = [1000000, 800000, 700000, 1000000, 800000, 700000]
    frac_angle = int(3*np.pi/4/delta_fi)
    frac_angle_2 = int(np.pi/delta_fi) + frac_angle
    frac_coord_1 = [1,2,3]
    frac_coord_2 = [1, 2, 3]
    frac_coords = [(1, frac_angle), (2, frac_angle), (3, frac_angle), (1, frac_angle_2), (2, frac_angle_2), (3, frac_angle_2)]
...
def sortByAngle(inputSet):
    ...
    print(Pres_distrib)
def PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3, c4, wells_coord, CP_dict, q, wells_frac_coords, delta_r, frac_coords, Pres, mu, perm, frac_angle, frac_angle_2, frac_pressure, frac_coord_1, frac_coord_2):
    ...
    for m in range(0, M_fi):
        ...
        if m == frac_angle:
            A[len(frac_coord_1)+1][len(frac_coord_1)] = 0
            B[len(frac_coord_1)+1][0] = B[len(frac_coord_1)+1][0] - (c1 - c2/(((wells_coord[i][0]+1)*delta_r)))*frac_pressure[len(frac_coord_1)-1]
            for i in range(len(frac_coord_1)-1, -1, -1):

                print(len(frac_coord_1), i)
                A = np.delete(A, frac_coord_1[i], axis=0)
                A = np.delete(A, frac_coord_1[i], axis=1)
                B = np.delete(B, frac_coord_1[i], axis=0)

        if m == frac_angle_2:
            A[len(frac_coord_2)+1][len(frac_coord_2)] = 0
            B[len(frac_coord_2)+1][0] = B[len(frac_coord_2)+1][0] - (c1 - c2/(((wells_coord[i][0]+1)*delta_r)))*frac_pressure[-1]
            for i in range(len(frac_coord_2)-1, -1, -1):
        ...
        if m == frac_angle:
            for i in range(len(frac_coord_1)):
                P_new = np.insert(P_new, frac_coord_1[i], frac_pressure[i])

        if m == frac_angle_2:
            for i in range(len(frac_coord_2)):
                P_new = np.insert(P_new, frac_coord_2[i], frac_pressure[len(frac_coord_1)+i])
...
if __name__ == '__main__':
    for t in range(T_exp):
        print(t)
        Pres_distrib = PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3, c4, wells_coord, CP_dict, q, wells_frac_coords, delta_r, frac_coords, Pres, mu, perm, frac_angle, frac_angle_2, frac_pressure, frac_coord_1, frac_coord_2)
