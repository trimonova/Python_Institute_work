...
wells_with_P = {(int((Lx/2+0.057)/hx),int((Ly/2+0.127)/hy)): 15*10**5, (int((Lx/2-0.057)/hx), int((Ly/2-0.127)/hy)): 1*10**5}
...
def PorePressure_in_Time():
    ...
    for t in range(1, T_exp):
        ...
        for m in range(1,M+1):
            ...
            for n in range(1,N+1):
                if n == 1  or n == N:
                    B[n-1][0] = -V*beta/t_step*Pres_distrib[n][m]- alpha*coeff_1*(Pres_distrib[n][m-1] - 2*Pres_distrib[n][m] + Pres_distrib[n][m+1]) - alpha* coeff_2*Pbound

                else:
                    B[n-1][0] = -V*beta/t_step*Pres_distrib[n][m]- alpha*coeff_1*(Pres_distrib[n][m-1] - 2*Pres_distrib[n][m] + Pres_distrib[n][m+1])
    ...
    #---------------------------------------------------------------------------
        ...
        for n in range(1, N + 1):
            ...
            for m in range(1, M + 1):
                if m == 1 or m == M:
                    B[m - 1][0] = -V * beta / t_step * Pres_distrib[n][m] - alpha * coeff_2 * (Pres_distrib[n-1][m] - 2 * Pres_distrib[n][m] + Pres_distrib[n+1][m]) - alpha * coeff_1 * Pbound

                else:
                    B[m - 1][0] = -V * beta / t_step * Pres_distrib[n][m] - alpha * coeff_2 *(Pres_distrib[n-1][m] - 2 * Pres_distrib[n][m] + Pres_distrib[n+1][m])
