...
#-----------------------------------------------------------------------------------
    ...
    for n in range(0, N_r):
        ...
        if n <= N_r_oil-1:
            A[0][0] = -2*c4/((n+1)*delta_r)**2 - c3_oil
            A[M_fi - 1][M_fi - 1] = -2 * c4 / ((n + 1) * delta_r) ** 2 - c3_oil
        if n > N_r_oil-1:
            A[0][0] = -2 * c4 / ((n + 1) * delta_r) ** 2 - c3_water
            A[M_fi - 1][M_fi - 1] = -2 * c4 / ((n + 1) * delta_r) ** 2 - c3_water
        A[0][1] = A[m][m+1]
        A[0][M_fi-1] = A[m][m-1]
...
if __name__ == '__main__':
    for t in range(T_exp):
        print(t)
        Pres_distrib = PorePressure_in_Time(N_r, M_fi, Pres_distrib, c1, c2, c3_oil, c3_water, c4, wells_coord, CP_dict, delta_r, P_center, N_r_oil)
        print(min(Pres_distrib.flat), max(Pres_distrib.flat))

    fig1 = plt.figure()
    print(np.shape(Pres_distrib))
    print(Pres_distrib[:, 30])
