...
fig = plt.figure()
surf = plt.contourf(xig, yig, Pi, cmap=cm.Greys, antialiased=True, vmin=np.nanmin(Pi), vmax=np.nanmax(Pi), linewidth=0.2,
                    levels=levels)
...
perm = 0.3 * 10 ** (-15)  # м2 проницаемость
k_water = mu_water*fi*(Cf+Cr)/perm
c3_water = k_water/delta_t
k_oil = mu_oil*fi*(Cf+Cr)/perm
c3_oil = k_oil/delta_t
...
for t in range(T_exp_1):
    print(t)
    Pres_distrib = PorePressure_in_Time_2(N_r, M_fi, Pres_distrib, c1, c2, c3_oil, c3_water, c4, wells_coord, CP_dict, delta_r, P_center[1500+t], N_r_oil)
...
frac_angle = round((4*np.pi/45)/delta_fi)
frac_angle_2 = round((43*np.pi/45)/delta_fi)
frac_coord_1 = [0,1,2,3]
frac_coord_2 = [0,1,2,3,4]
...
perm = 0.75* 10 ** (-15)  # м2 проницаемость
k_water = mu_water*fi*(Cf+Cr)/perm
c3_water = k_water/delta_t
k_oil = mu_oil*fi*(Cf+Cr)/perm
c3_oil = k_oil/delta_t
...
for t in range(T_exp_2):
    print(t)
    P_center = set_with_P_mesh[(0,0)][1500+T_exp_1+t]

    frac_pressure_1 = [P_center/4 for i in range(len(frac_coord_1))]
    frac_pressure_2 = [P_center/4 for i in range(len(frac_coord_2))]
    frac_pressure_1[-1] = P_well[0] / 2
    frac_pressure_2[-1] = P_well[0] / 2
    ...
    for i in range(len(frac_coords)):
        CP_dict[frac_coords[i]] = frac_pressure[i]
    Pres_distrib = PorePressure_in_Time_3(N_r, M_fi, Pres_distrib, c1, c2, c3_water, c3_oil, c4, wells_coord, CP_dict, delta_r, frac_coord_1, frac_coord_2, frac_angle, frac_angle_2, xf, wells_frac_coords, frac_pressure_1, frac_pressure_2)
...
for coord in set_with_P_mesh:
    ...
    plt.plot(set_with_P_mesh[coord][1500:5000] - set_with_P_mesh[coord][1499])
