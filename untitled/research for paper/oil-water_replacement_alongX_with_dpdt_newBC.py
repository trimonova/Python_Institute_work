# прямолиненйное вытеснение воды маслом. На границе задается постоянное давление, равное атмосферному.
# решается нестационарное уравнение пьезопроводности с одной координатой.
import numpy as np
...
for time in range(T):
    ...
    A[N_oil+N_water - 2][N_oil+N_water - 2] = -(2 + k_water*h_water**2/t_step)
    A[N_oil+N_water - 2][N_oil+N_water - 3] = 1
    B[N_oil+N_water-2][0] = -k_water*h_water**2/t_step * P_old[N_oil+N_water-2] - P_bound
