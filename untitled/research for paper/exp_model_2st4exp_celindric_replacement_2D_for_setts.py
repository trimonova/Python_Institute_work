...
perm = 0.05 * 10 ** (-15)  # м2 проницаемость
k_water = mu_water*fi*(Cf+Cr)/perm
c3_water = k_water/delta_t
k_oil = mu_oil*fi*(Cf+Cr)/perm
c3_oil = k_oil/delta_t
...
for t in range(T_exp_1):
    print(t)
    Pres_distrib = PorePressure_in_Time_2(N_r, M_fi, Pres_distrib, c1, c2, c3_oil, c3_water, c4, wells_coord, CP_dict, delta_r, P_center[4500+t], N_r_oil)
   #print(P_center[4500+t]/4)
...
frac_angle = round((np.pi/20*11)/delta_fi)
frac_angle_2 = round((np.pi + np.pi/20*11)/delta_fi)
...
for t in range(T_exp_2):
    print(t)
    P_center = set_with_P_mesh[(0,0)][4500+T_exp_1+t]
    frac_pressure_1 = [P_center/2 for i in range(len(frac_coord_1))]
    frac_pressure_2 = [P_center/2 for i in range(len(frac_coord_2))]
...
for coord in set_with_P_mesh:
    ...
    plt.plot(set_with_P_mesh[coord][4500:9000] - set_with_P_mesh[coord][4499])
