...
perm = 0.01 * 10 ** (-15)  # м2 проницаемость
k = mu*fi*(Cf+Cr)/perm
c3 = k/delta_t
q = 0.00003
...
frac_pressure = [4000000, 4000000, 4000000,4000000, 4000000, 4000000, 4000000]
frac_angle = 25
frac_angle_2 = 70
...
time = [i/100 for i in range(4000)]
time1 = [i/100-10 for i in range(4000)]
for coord in set_with_P_mesh:
    fig = plt.figure()

    plt.plot(time, set_with_P_mesh[coord][5000:9000] - set_with_P_mesh[coord][4000])
    plt.plot(time, set_with_P_mesh_theory[coord] - set_with_P_mesh_theory[coord][0],'r', linewidth = 2)
    #plt.title((coord[0]*delta_r, coord[1]*delta_fi))
    plt.xlabel('Time, s')
    plt.ylabel('Pressure increase, Pa')
    plt.axis((0, 40, -500,100000))
    plt.show()
