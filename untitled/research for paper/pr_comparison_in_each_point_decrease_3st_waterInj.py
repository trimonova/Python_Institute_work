...
for couple in set_coord:
    set_with_P[couple] = pt[i][900000:1300000]
    i += 1
...
wells_with_P = {}
wells_with_P[(xhole[0][0], yhole[0][0])] = Pprod
wells_with_P[(xhole[2][0], yhole[2][0])] = 0
wells_with_P_mesh = {}
for key in wells_with_P:
    wells_with_P_mesh[(int((key[0]+Lx/2)/hx), int((key[1]+Ly/2)/hy))] = wells_with_P[key]
...
time_1 = np.arange(0, 400000)/100
time_2 = np.arange(0, 2000)/1


for key in set_with_P_mesh:
    print(key)
    fig = plt.figure()
    plt.axis([0, 1500, 0, 1000000])
