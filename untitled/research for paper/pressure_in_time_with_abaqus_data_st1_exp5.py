...
fig2 = plt.figure()
xig,yig,zi = pressure_in_abaqus('PorePr_from_2stExp/PorePr_2st_5exp.csv')
levels = list(range(0,Pinj+10**5,50000))
surf = plt.contourf(xig, yig, zi, cmap=cm.Greys, antialiased=True, vmin=np.nanmin(zi), vmax=np.nanmax(zi),linewidth=0.2, levels = levels)
fig2.colorbar(surf, shrink=0.5, aspect=5)

x, y, p_in_time, xig_2, yig_2, Pini = pressure_in_exp('../Давление/data5.mat')
zi_2 = interpolate.griddata((x[0], y[0]), Pini, (xig_2, yig_2), method='cubic')
plt.contourf(xig_2, yig_2, zi_2, cmap=cm.Greys, antialiased=True, vmin=np.nanmin(zi), vmax=np.nanmax(zi), linewidth=0.2, levels = levels)
plt.scatter(x,y)
plt.xlabel("X, m")
plt.ylabel("Y, m")
plt.show()
